...
/** Generated class from Pigeon that represents data sent in messages. */
data class ExposureTimeRange (
  val minMicroseconds: Long,
  val maxMicroseconds: Long
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): ExposureTimeRange {
      val minMicroseconds = pigeonVar_list[0].let { num -> if (num is Int) num.toLong() else num as Long }
      val maxMicroseconds = pigeonVar_list[1].let { num -> if (num is Int) num.toLong() else num as Long }
      return ExposureTimeRange(minMicroseconds, maxMicroseconds)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
interface CameraInterface {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: CameraInterface?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.camerawesome.CameraInterface.setExposureTime$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
